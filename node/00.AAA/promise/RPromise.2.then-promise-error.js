...
class RPromise {
  ...
  then(onFulfilled, onRejected) {
    ...
    if (this.status == FULFILLED) {
      ...
      promise2 = new RPromise((resove, reject) => {
        try { //使用try    进行连续抛错  在第一个 then中的onFulfilled中 抛到第二个then中的 onRejected
          let x = onFulfilled(this.value); //可能是 普通值  promise
          // 如果获取到返回值x  会走解析promise的过程 实现自己的promise 和别人的交互
    ...
    if (this.status == REJECTED) {
      return (promise2 = new RPromise((resove, reject) => {
        try {
    ...
    // 2.2.4 如果当前promise状态未知  则把状态先 装进 数组 以便未来调用  （异步的时候）
    if (this.status == PENDING) {
      return (promise2 = new RPromise((resove, reject) => {
